...
async def check_routing(state: LegalState) -> dict:
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data, privacy, GDPR, CCPA, consent, or data breaches'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
    ...
    privacy_keywords = ["data", "privacy", "gdpr", "ccpa", "consent", "breach", "dữ liệu"]
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    messages = [
        SystemMessage(
            content=(
                "You are a data privacy attorney specialising in GDPR, CCPA/CPRA, "
                "data protection, consent, breach notification, and privacy rights. "
                "Analyse only the privacy and data-protection issues. Keep your response under 200 words."
            )
        ),
        HumanMessage(
            content=(
...
QUESTION = (
    ...
    "what are the legal, tax, regulatory, and privacy consequences?"
)
